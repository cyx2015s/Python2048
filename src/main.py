...
CUBE_KEYS = {"w": [0, False], "e": [1, False], "d": [2, False], "s": [3, False], "a": [4, False], "q": [5, False]}
...
def special_vec_valid(vec_pos, size):
    return (0 <= vec_pos[0] < size and 0 <= vec_pos[1] < size) and (
            (vec_pos[0] != size // 2 and vec_pos[1] != size // 2) or (
            vec_pos[0] == 0 or vec_pos[0] == size - 1 or vec_pos[1] == 0 or vec_pos[1] == size - 1))


games = [core.Core(cube.Cube, cube.CUBE_SIDES, cube_valid,
                   CUBE_KEYS, i * 2 - 1) for i in range(4, 7)] + [
            core.Core(vec.Vec, vec.VEC_SIDES, vec_valid,
                      VEC_KEYS, i) for i in range(3, 9)] + [
            core.Core(vec.Vec, vec.VEC_SIDES, special_vec_valid, VEC_KEYS, 7)]
game_texts = [f"Hexagon (size={str(i)})" for i in range(3, 6)] \
...
for h_start in [100]:
    for h_delta in [(HEIGHT - h_start) / len(game_texts)]:
        for i in range(len(game_texts)):
            buttons.append(activator.Activator(
                pygame.rect.Rect((HEIGHT+10, h_start + i * h_delta), (WIDTH - HEIGHT, h_delta)).inflate(-20, -5),
                switch_to_board, index=i))
reset_button=activator.Activator(pygame.rect.Rect((HEIGHT+20,10),(100,80)),reset_board,index=None)
...
def draw_game_grid(iter, name, size=None):
    ...
    if type(iter) == cube.Cube:
        ...
        draw_flat_hexagon(tmp_center, size - 2,
                          color=coloring.get_color(name)[1])
        if name is not None:
            text.drawbox(str(name), fontname=CONSOLA_TTF,
                         rect=pygame.rect.Rect(tmp_center - vec.Vec(size, size), size * vec.Vec(2, 2)),
                         color=coloring.get_color(name)[0])
    elif type(iter) == vec.Vec:
        tmp_topleft = TOPLEFT + iter * SIZE * 2
        pygame.draw.rect(screen, coloring.get_color(name)[1],
                         pygame.rect.Rect(tmp_topleft, 2 * vec.Vec(SIZE, SIZE)).inflate(-5, -5).inflate(
                             2 * (size - SIZE), 2 * (size - SIZE)))
        if name is not None:
            text.drawbox(str(name), fontname=CONSOLA_TTF,
                         rect=pygame.rect.Rect(tmp_topleft, 2 * vec.Vec(SIZE, SIZE)).inflate(-20, -30).inflate(
                             size - SIZE, size - SIZE),
                         color=coloring.get_color(name)[0])
...
def draw():
    ...
    text.draw("Score:\n%8d" % game.score, topright=(WIDTH, 5), fontname=CONSOLA_TTF, fontsize=36)
    draw_sidebar()
...
def draw_anim_game_board():
    ...
    for anim in game.lst_anims[cur_state]:
        if type(anim) == core.AnimMove:
            draw_game_grid((anim.iter * cur_time + anim.from_iter * (1 - cur_time)), anim.name, SIZE)
        elif type(anim) == core.AnimMerge:
            draw_game_grid(anim.iter, anim.name, SIZE * (1 + 0.5 * cur_time * (1 - cur_time)))
...
def draw_sidebar():
    for i in range(len(buttons)):
        button=buttons[i]
        if i == game_id:
            pygame.draw.rect(screen,coloring.get_color(4)[1],button.rect.inflate(4,4))
        pygame.draw.rect(screen,coloring.get_color(None)[1],button.rect)
        text.draw(game_texts[i], topright=vec.Vec(0,5)+button.rect.topright, fontname=CONSOLA_TTF)
